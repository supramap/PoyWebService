...
namespace PoyService
{
    public class Poy
    {
        ...
        public int Init(int dir)
        {
            ...
            ssh.Write("mkdir ~/poy_service/"+dir.ToString());
            return dir;
        }
		...
		public bool AddTextFile(int jobId, string fileData, string filename)
        {
            ...
            shell.Write(string.Format("mv {0} ~/poy_service/{1}/{0}", filename, jobId.ToString()));
        ...
        public string Submit(int jobId, int numberOfNodes, int wallTimeHours, int wallTimeMinutes)
        {
            ...
            using (StreamWriter writer = new StreamWriter(dir, false))
            {
                ...
                writer.Write("PATH=$PATH:/nfs/17/osu6013/bin:/nfs/17/osu6013/lib \n");
                writer.Write(string.Format("cd /nfs/17/osu6013/poy_service/{0}/ \n", jobId));
            ...
            sftp.Close();
            SshStream shell = getShell();
            shell.Write(string.Format("mv {0} ~/poy_service/{1}/{0} ;", "batch.job", jobId.ToString()));
            shell.Write(string.Format("cd ~/poy_service/{0} ;",  jobId.ToString()));
        ...
        public bool isDone(int jobId)
        {
            ...
            commander.RunCommand(
                string.Format(@"[ -f ~/poy_service/{0}/poy_{0}.* ] && echo 'yes' || echo 'no'", jobId),
                ref output,
                ref error);

            if (output == "yes\n")
            {
                commander.RunCommand(string.Format(@"cp ~/poy_service/{0}/poy_{0}.* ~/poy_service/{0}/output.txt", jobId));
                return true;
            }
            else
                return false;
        ...
        public byte[] getFile(int jobId, string fileName)
        {
            ...
            sftp.Get(
                string.Format(@"poy_service/{1}/{0}", fileName, jobId),
